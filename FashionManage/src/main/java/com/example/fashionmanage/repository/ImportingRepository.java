...
public interface ImportingRepository extends JpaRepository<Importing, Integer> {
        ...
        @Query(value = "WITH CombinedData AS ( " +
                        ...
                        "SELECT " +
                        "DATE(import_date) AS date, " +
                        "0 AS total_revenue," + "total AS total_expenditure " + "FROM importing "
                        + ") AS combined_data "
                        + "WHERE DATE(date) = :date " + "GROUP BY DATE(date) " + ") " + "SELECT " + "transaction_date, "
                        + "MAX(total_revenue) AS total_revenue, " + "MAX(total_expenditure) AS total_expenditure "
                        + "FROM CombinedData "
